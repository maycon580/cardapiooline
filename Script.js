...
const closeModalBtn = document.getElementById("class-modal-btn");
...
// Adiciona função para remover itens
...
function removeFromCart(name) {
   ...
   if (index !== -1) {
      ...
      if (item.quantity > 1) {
         // Se houver mais de uma unidade, diminui a quantidade em 1
         item.quantity -= 1;
      } else {
...
// Adiciona o evento de input para o campo de endereço
addressInput.addEventListener("input", function (event) {
   let inputvalue = event.target.value; // Corrigido para 'value' em vez de 'Value'
   if(inputvalue !== ""){
...
checkoutBtn.addEventListener("click", function() {
   ...
   const isOpen = checkRestauramtOpen();
   if(!isOpen){
   ...
   if (addressInput.value === "") {
      ...
      addressInput.classList.add("border-red-500"); // Adicione esta linha
      return;
   }
   //enviar via whatsap
